...
function App() {
  ...
  if (loading) return <div className="h-screen bg-slate-950 flex items-center justify-center text-blue-500 font-black animate-pulse uppercase">Support.Net Loading...</div>;
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      <nav className="w-64 bg-slate-900 border-r border-white/5 flex flex-col shrink-0">
        ...
        <div className="flex-1 px-4 space-y-2">
          <window.NavBtn active={view === 'home'} onClick={() => setView('home')} icon={<Home size={20}/>} label="Лента" />
          {profile.role === 'admin' && (
            <window.NavBtn active={view === 'admin'} onClick={() => setView('admin')} icon={<ShieldCheck size={20}/>} label="Админка" />
          )}
          <window.NavBtn active={view === 'profile'} onClick={() => setView('profile')} icon={<Settings size={20}/>} label="Профиль" />
        </div>
        <div className="p-6 border-t border-white/5 flex items-center gap-4">
           ...
           <button onClick={() => window.auth.signOut()} className="text-slate-500 hover:text-red-500 transition-colors"><LogOut size={16}/></button>
        </div>
      </nav>
                ...
                <div className="bg-slate-900/80 p-6 rounded-[2rem] border border-blue-500/20 shadow-xl">
                  ...
                  <button onClick={async () => {
                    ...
                    await window.db.collection('artifacts').doc(window.appId).collection('public').doc('data').collection('news').add({ 
                      text: newPostText, author: profile.name, createdAt: firebase.firestore.FieldValue.serverTimestamp(), likes: [] 
                    });
                    setNewPostText('');
          ...
          {view === 'admin' && (
            <div className="space-y-8">
              ...
              <div className="grid gap-4">
                {allUsers.map(u => (
                  <div key={u.uid} className="bg-slate-900/60 p-6 rounded-[2rem] border border-white/5 flex items-center justify-between">
            ...
            <div className="max-w-md mx-auto text-center py-10">
              <div className="bg-slate-900 p-10 rounded-[4rem] border border-white/5 shadow-2xl">
                ...
                <div className="mt-12 text-left">
                  <label className="text-[10px] font-black uppercase text-slate-500 ml-5 tracking-widest italic">Ваше имя</label>
                  <input 
                    className="w-full bg-slate-950 p-5 rounded-[1.5rem] border border-white/5 outline-none focus:border-blue-500/50 text-white font-bold" 
                    value={profile.name}
                    onChange={e => {
