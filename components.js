window.RoleBadge = ({ role }) => {
  ...
  return React.createElement('span', {
    className: `px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-tighter ${styles[role] || styles.user}`
...
window.NavBtn = ({ active, onClick, icon, label }) => {
  return React.createElement('button', {
    ...
    React.createElement('div', { className: `transition-transform ${active ? 'scale-110' : 'group-hover:scale-110'}` }, icon),
    React.createElement('span', { className: "font-black text-[10px] uppercase tracking-widest" }, label),
    active && React.createElement('div', { className: "absolute right-4 w-1.5 h-1.5 bg-white rounded-full animate-pulse shadow-[0_0_8px_white]" })
  );
};
